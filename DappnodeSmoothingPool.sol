...
contract DappnodeSmoothingPool is OwnableUpgradeable {
    ...
    function initialize(
        ...
        deploymentBlockNumber = uint128(block.number); // Gas savings
    ...
    function OnlyGovernance1() view internal {//Gas saving
        require(
            governance == msg.sender,
            "DappnodeSmoothingPool::onlyGovernance: Only governance"
        );
    ...
    function subscribeValidators(
        ...
        // Check collateral
        uint256 lscl = subscriptionCollateral; //Gas saving
        require(
            msg.value == lscl * validatorIDArray.length,
            "DappnodeSmoothingPool::subscribeValidator: msg.value does not equal subscription collateral"
        );

        uint256 i; //Gas saving
        do {
            emit SubscribeValidator( //Gas saving
                msg.sender,
                lscl, //Gas saving
                validatorIDArray[i]
            );
        ...
        } while (i < validatorIDArray.length); //Gas saving
    }
    ...
    ) external payable onlyOwner {//Gas saving
     ...
     */
    function updatePoolFee(uint16 newPoolFee) external payable onlyOwner {//Gas saving    
        require(
            newPoolFee <= 10000,
    ...
    function getOracleMemberIndex(
        ...
        uint256 OracleLength = oracleMembers.length; //Gas saving
        ...
        } while (i < OracleLength); //Gas saving
    ...
    ) public pure returns (bytes32 reportHash) {
